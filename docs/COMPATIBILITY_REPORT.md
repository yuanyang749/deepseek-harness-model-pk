...
| Execution | macOS uses deny-default Seatbelt plus a process group. Windows uses an AppContainer with the outbound-network capability, explicit Attempt ACLs and a kill-on-close Job Object. The probe proves workspace writes and loopback network access while denying sibling reads, shared temp, secret environment and background orphans. |
